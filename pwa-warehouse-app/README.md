...
- **オフラインOCR**: Tesseract.jsによる日本語OCR機能（オンデマンド読み込み）
...
- HTTPSに対応したWebサーバー、またはローカル開発用のHTTPサーバー
- モダンなWebブラウザ（Chrome 67+, Firefox 63+, Safari 11.1+, Edge 79+）
...
4. ブラウザで以下のURLにアクセス:
   - ログイン: `http://localhost:8000/pwa-warehouse-app/login/index.html`
   - 入出庫記録: `http://localhost:8000/pwa-warehouse-app/intake-delivery/index.html`

### 🔐 GAS APIエンドポイント

現在設定されているエンドポイント:

- **ログインAPI**: `https://script.google.com/macros/s/AKfycbzSmWPMPsCktP1c8AHxX-0_3QHTHglpygSPQvnhTOV0rYjGwnhkQgwHjNitQ7iheU27/exec`
- **入出庫記録API**: `https://script.google.com/macros/s/AKfycbxnm6ga-jK6MZ6IYI83RwVcEoSssZCIueiuf0LIfKmlFOtR-pbS0hCg2Q52TLHvDDtJ9A/exec`

エンドポイントを変更する場合:
- ログイン: `login/js/app.js` の `GAS_LOGIN_API_URL` を更新
- 入出庫記録: `intake-delivery/js/app.js` の `GAS_INTAKE_API_URL` を更新
- 同期マネージャー: `shared/js/sync-manager.js` の `baseUrl` を更新
